...
CREATE MATERIALIZED VIEW epu_running_daily AS
  ...
  SELECT
    instrument_id,
    time,
    toolkit_experimental.interpolated_duration_in(agg, se.running_value, time, '1 day',
                                                  LAG(agg) OVER ( PARTITION BY instrument_id ORDER BY time )) AS running_duration
  FROM
    events.epu_state_daily
    JOIN state_enum se
      USING (instrument_id)
  ORDER BY
    instrument_id,
