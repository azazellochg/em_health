...
CREATE MATERIALIZED VIEW IF NOT EXISTS events.tomo_counters AS
    ...
    FROM
        ...
        JOIN LATERAL (
            ...
            SELECT COALESCE(sum(rp.peak), 0) + COALESCE(max(fp.peak), 0) AS total_image_counter
            FROM
                reset_peaks rp
                FULL JOIN final_peak fp ON true
        ) img_agg ON true
        JOIN LATERAL (
            WITH seg_data AS (
                SELECT
            ...
            SELECT COALESCE(sum(peak), 0) AS total_ts_counter
            FROM reset_peaks
        ) ts_agg ON true
        JOIN LATERAL (
            WITH seg_data AS (
            ...
            SELECT COALESCE(sum(peak), 0) AS total_sm_counter
            FROM reset_peaks
        ) sm_agg ON true
