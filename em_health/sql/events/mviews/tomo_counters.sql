...
CREATE MATERIALIZED VIEW IF NOT EXISTS events.tomo_counters AS
  WITH
    image_counter_param AS (
      SELECT
        instrument_id,
        param_id AS image_counter_param_id
      FROM
        events.parameters
      WHERE
    ...
    ts_counter_param AS (
      SELECT
        instrument_id,
        param_id AS ts_counter_param_id
      FROM
        events.parameters
      WHERE
    ...
    sm_counter_param AS (
      SELECT
        instrument_id,
        param_id AS sm_counter_param_id
      FROM
        events.parameters
      WHERE
  ...
  SELECT
    ...
    seg.session_id,
    seg.start_time,
    img_agg.total_image_counter::INT,
    ts_agg.total_ts_counter::INT,
    sm_agg.total_sm_counter::INT
  FROM
    events.tomo_sessions seg
    JOIN image_counter_param ic
      ON ic.instrument_id = seg.instrument_id
    LEFT JOIN ts_counter_param ts
      ON ts.instrument_id = seg.instrument_id
    LEFT JOIN sm_counter_param sm
      ON sm.instrument_id = seg.instrument_id
    JOIN LATERAL ( WITH
                     seg_data AS (
                       SELECT
                         d.time,
                     ...
                     reset_peaks AS (
                       SELECT prev_value AS peak FROM seg_data WHERE value_num = 1 AND prev_value IS NOT NULL
                     ),
                     final_peak AS (
    ...
    JOIN LATERAL ( WITH
                     ...
                     reset_peaks AS (
                       SELECT prev_value AS peak FROM seg_data WHERE value_num = 0 AND prev_value IS NOT NULL
                     )
                   SELECT
