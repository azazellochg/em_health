...
*/
CREATE MATERIALIZED VIEW IF NOT EXISTS events.tomo_runs AS
  WITH
    state_param AS (
      SELECT
        instrument_id,
    ...
    state_enum AS (
      ...
      FROM
        state_param p
        JOIN events.enum_values e
          ON e.enum_id = p.enum_id
      WHERE
        e.member_name IN ('Running', 'Acquiring', 'Terminated')
      GROUP BY
        p.instrument_id,
        p.param_id
    ),
    runs AS (
      SELECT
        ...
        SUM(CASE WHEN v.state = se.running_value THEN v.duration ELSE '0 second'::INTERVAL END) AS running_duration,
      ...
      FROM
        ...
        JOIN state_enum se
          USING (instrument_id)
        JOIN state_param sp
          ON se.instrument_id = sp.instrument_id AND se.param_id = sp.param_id
        JOIN LATERAL ( SELECT
                         state,
                         duration
                       FROM
                         toolkit_experimental.into_int_values((
                           SELECT
                             toolkit_experimental.compact_state_agg(d.time, d.value_num::BIGINT)
                           FROM
                             events.data d
                           WHERE
  ...
  FROM
    runs
  WHERE
    running_duration > '0 second'::INTERVAL
