...
CREATE MATERIALIZED VIEW IF NOT EXISTS em_off AS
    WITH server_param AS (
        ...
        SELECT
            d.instrument_id, d.time, d.value_num,
            lead(d.time) OVER (
        ...
        FROM
            data d
            JOIN server_param sp ON d.instrument_id = sp.instrument_id
            AND d.param_id = sp.param_id
    )
    SELECT instrument_id, time AS start_time, next_time AS end_time
    FROM server_events
    WHERE
        value_num = 0
