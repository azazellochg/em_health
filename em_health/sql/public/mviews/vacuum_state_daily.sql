...
CREATE MATERIALIZED VIEW IF NOT EXISTS vacuum_state_daily AS
    ...
    enum_states AS (
        SELECT
            vp.instrument_id, e.value AS enum_value,
            CASE
                WHEN e.member_name IN ('ColumnConditioning', 'Column Conditioning', 'TMPmOnColumn', 'CryoCycle', 'Cryo Cycle', 'CryoCycle_Time', 'CryoCycle_Delay') THEN 'cryocycle'
                WHEN e.member_name IN ('All Vacuum [Closed]', 'AllVacuumColumnValvesClosed', 'AllVacuum_LinersClosed') THEN 'closed'
                WHEN e.member_name IN ('All Vacuum [Opened]', 'AllVacuumColumnValvesOpened', 'AllVacuum_LinersOpened') THEN 'open'
                ELSE 'unknown'
            END AS state
        FROM
            enum_values e
    ...
    vacuum_events AS (
        SELECT
            d.instrument_id, d.time AS start_time,
            lead(d.time) OVER (
    ...
    all_states AS (
        SELECT instrument_id, start_time, end_time, state
        FROM cleaned_vacuum
        UNION ALL
    ...
    split_intervals AS (
        SELECT
            instrument_id, state, gs::date AS day,
