...
SELECT
    ...
    d.instrument_id,
    MAX(CASE WHEN p.param_name IN ('LoadCartridgeCounter', 'SampleTransferCount') THEN d.value_num END)
        - MIN(CASE WHEN p.param_name IN ('LoadCartridgeCounter', 'SampleTransferCount') THEN d.value_num END) AS daily_cartridge_count,
