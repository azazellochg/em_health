    ...
    IF current_version = 1 THEN
      ...
      SELECT
        EXISTS (
          ...
          WHERE
            table_schema = 'pganalyze'
            AND table_name = 'stat_statements'
            AND column_name = 'userid'
        )
      INTO col_exists;

      IF NOT col_exists THEN ALTER TABLE pganalyze.stat_statements
        ADD COLUMN
          userid oid;

        UPDATE pganalyze.stat_statements SET userid = grafana_oid;
      ...
      IF col_type IS NOT NULL AND col_type <> 'bigint' THEN ALTER TABLE pganalyze.stat_statements
        DROP COLUMN wal_bytes; ALTER TABLE pganalyze.stat_statements
        ADD COLUMN
      ...
      SELECT
        EXISTS (
          SELECT 1
          FROM pg_roles
          WHERE rolname = 'emhealth'
        )
      INTO role_exists;

      IF NOT role_exists THEN CREATE ROLE emhealth WITH LOGIN PASSWORD 'emhealth';
      END IF;
      ...
      ALTER TABLE data
        DROP CONSTRAINT data_time_instrument_id_param_id_key;
      ALTER TABLE data
        ADD UNIQUE (param_id, instrument_id, time);
